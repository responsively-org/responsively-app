...
export const Suite = ({suite: {id, name, devices}, isActive}: Props) => {
  ...
  return (
    <div
      className={cx('relative min-w-56 shrink-0 rounded bg-white dark:bg-slate-900', {
        'border-2 border-slate-500': isActive,
      })}
    >
      {!isActive ? (
        <div className="absolute flex h-full w-full items-center justify-center bg-gray-100/70 dark:bg-slate-800/70">
          <Button
