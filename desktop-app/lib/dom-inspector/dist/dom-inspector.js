...
(function(global, factory) {
    ...
    function getMaxZIndex() {
        return []
            .concat(toConsumableArray(document.all))
            .reduce(function(r, e) {
    ...
    proxy.forEach(function(item) {
        exportObj[item] = function funcName() {
            return console[item].call(
                this,
                sep + (arguments.length <= 0 ? undefined : arguments[0]),
    ...
    var DomInspector = (function() {
        function DomInspector() {
            ...
            if (isNull(this.root)) {
                exportObj.warn(
                    'Root element is null. Auto select body as root'
                );
                this.root = $('body');
            }
            ...
            },
            {
                ...
                value: function _init() {
                    ...
                    this.overlay = {
                        ...
                        paddingTop: this._createSurroundEle(
                            parent,
                            'padding padding-top'
                        ),
                        ...
                        paddingLeft: this._createSurroundEle(
                            parent,
                            'padding padding-left'
                        ),
                        borderTop: this._createSurroundEle(
                            parent,
                            'border border-top'
                        ),
                        borderRight: this._createSurroundEle(
                            parent,
                            'border border-right'
                        ),
                        ...
                        borderLeft: this._createSurroundEle(
                            parent,
                            'border border-left'
                        ),
                        marginTop: this._createSurroundEle(
                            parent,
                            'margin margin-top'
                        ),
                        marginRight: this._createSurroundEle(
                            parent,
                            'margin margin-right'
                        ),
                        ...
                        marginLeft: this._createSurroundEle(
                            parent,
                            'margin margin-left'
                        ),
            ...
            },
            {
                ...
                value: function _onMove(e) {
                    for (var i = 0; i < this.exclude.length; i += 1) {
                        ...
                        if (
                            cur.isEqualNode(e.target) ||
                            isParent(e.target, cur)
                        )
                            return;
                    }
                    ...
                    $('.id', this.overlay.tips).innerHTML = this.target.id
                        ? '#' + this.target.id
                        : '';
                    $('.class', this.overlay.tips).innerHTML = []
                        .concat(toConsumableArray(this.target.classList))
                        .map(function(item) {
                            return '.' + item;
                        })
                        .join('');
                    $('.size', this.overlay.tips).innerHTML =
